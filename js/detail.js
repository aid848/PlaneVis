...
class Detail {
  ...
  initVis() {
    ...
    vis.sizeScaleInverted = d3
      .scaleLinear()
      .range([vis.maxCircleSize, vis.minCircleSize]);
  ...
  updateVis() {
    ...
    vis.sizeScale.domain([
      vis.dataGrouped[vis.maxElements - 1][1],
      vis.dataGrouped[0][1],
    ]);
    vis.sizeScaleInverted.domain([
      vis.dataGrouped[0][1],
      vis.dataGrouped[vis.maxElements - 1][1],
    ]);
    d3.selectAll(vis.title).text(
      `${secondary_selector} by Aircraft Model (${vis.selected})`
    );
    vis.renderVis();
  }
  ...
  renderVis() {
    ...
    vis.circles = vis.node
      .append("image")
      .attr("xlink:href", (d) => {
        return vis.planeConfigToImage(d[0]);
      })
    ...
    vis.sim = d3
      .forceSimulation(vis.dataGrouped, (d) => [d[0], vis.sizeScale(d[1])])
  ...
  drag(d) {
    // remove tooltip and allow plane to move by mouse
    d3.select("#tooltip").style("display", "none");
    d.subject.x = d.x;
    d.subject.y = d.y;
  }
