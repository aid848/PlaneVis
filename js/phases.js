class FlightPhase {
  constructor(_config, _data, _dispatcher) {
    ...
    this.validPhasePoints = this.flightPathPoints.filter(
      (d) => d.phase !== null
    );
    this.data = _data;
  ...
  initVis() {
    ...
    vis.flightPathGenerator = d3
      .line()
      .x((d) => d.x)
      .y((d) => d.y)
      .curve(d3.curveBasis);

    vis.flightCategories = d3
      .scaleOrdinal()
      .domain(this.validPhasePoints.map((d) => d.phase));

    // set the color scale
    vis.globalColor = d3.scaleOrdinal().domain(["Commercial", "Personal"]);
    // .range(["#0024d6", "#d60004"]);

    vis.pie = d3.pie();

    // shape helper to build arcs:
    vis.arcGenerator = d3.arc().innerRadius(0).outerRadius(70);
    ...
    vis.animationPath = vis.pathView
      .append("path")
    ...
    phaseG
      ...
      .attr("dy", 30)
      .attr("transform", (d) => {
        if (d.phase === "Climb") return `translate(${d.x}, ${d.y}) rotate(-53)`;
        else if (d.phase === "Approach")
          return `translate(${d.x}, ${d.y}) rotate(45)`;
        return `translate(${d.x}, ${d.y})`;
      })
      .attr("text-anchor", (d) => {
        if (d.phase === "Start") return "start";
        else if (d.phase === "Summary") return "end";
        return "middle";
      })
      .text((d) => d.phase);

    // flight marker to be animated along the path
    vis.fightMarker = vis.pathView
      .append("image")
      .attr("xlink:href", "figs/icons8-fighter-jet-48.png")
      .attr("y", -10);

    vis.pieGroup = vis.pathView.append("g").attr("class", "pie-pathView-group");
  }
  ...
  updateVis(forward = true, nextStop = -1) {
    ...
    if (nextStop !== -1) {
      ...
      const pathBegin =
        nextStop === 0
          ...
          : vis.flightPathPoints.findIndex(
              (stop, idx) =>
                stop.phase === vis.validPhasePoints[nextStop - 1].phase
            );
      ...
      vis.animatedStops = forward
        ? vis.animatedStops
        : vis.animatedStops.reverse();
  ...
  renderVis(forward, reachSummary, nextStop) {
    ...
    vis.animationPath.attr("d", vis.flightPathGenerator(vis.animatedStops));
        ...
        if (nextStop !== -1 && !reachSummary) {
          if (forward) {
            vis.dispatcher.call(
              "filterPhaseData",
              event,
              vis.phaseNameUpperCase[nextStop]
            );
          } else {
            vis.dispatcher.call(
              "filterPhaseData",
              event,
              vis.phaseNameUpperCase[nextStop - 1]
            );
          }
        }
      });
    ...
    if (forward && reachSummary) {
      ...
      const top = d3.select("#summary-container").node().getBoundingClientRect()
        .top;
      ...
      let pieInfo = pieG
        ...
        .data(
          (d) => {
            const phaseData = d[1];
            const personalData =
              phaseData[0][0] === true ? phaseData[0] : phaseData[1];
            const commercialData =
              phaseData[0][0] === true ? phaseData[1] : phaseData[0];
            const data = {
              Commercial: commercialData[1].length,
              Personal: personalData[1].length,
            };
            const keyValuePair = Array.from(
              Object.entries(data),
              ([key, value]) => ({ key, value })
            );
