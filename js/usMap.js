class UsMap {
  ...
  updateVis(bubble) {
    ...
    vis.filteredData = vis.data.filter(
      (d) => vis.projection([d.Longitude, d.Latitude]) != null
    );

    vis.hexData = vis
      .hexbin(vis.filteredData)
      .map((d) => ((d.binMetric = d3.sum(d, (d) => d[vis.attribute])), d))
      .sort((a, b) => b.length - a.length);

    vis.color = d3.scaleSequential(
      d3.extent(vis.hexData, (d) => d3.sum(d, (d) => d[vis.attribute])),
      d3.interpolateCividis
    );
    vis.radius = d3.scaleSqrt(
      [0, d3.max(vis.hexData, (d) => d.length)],
      [0, vis.hexbin.radius() * Math.SQRT2]
    );
  ...
  renderVis(bubble) {
    ...
    if (vis.hexData.length !== 0) {
      ...
      vis.chart.attr("opacity", 0.45);

      // Append hexbin
      const hexbin = vis.svg
        .selectAll(".hex-bin-path")
      ...
      vis.chart.attr("opacity", 0.1);
      const label = vis.svg
        .selectAll(".no-location-text")
        .data([bubble])
        .join("text")
        .style("font-size", 17)
      ...
      if (bubble === undefined) {
        label.text("No location data available for this date range");
      } else if (bubble !== null) {
        label.text("No location data available for " + bubble);
      } else {
