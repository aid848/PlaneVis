...
// setup dispatchers
const control_panel_dispatcher = d3.dispatch(
  "control_filter",
  "overview_click",
  "detail_click"
);
...
Promise.all([joined_data_p, ac_data_p, ntsb_data_p, us_map_data_p])
  .then((data) => {
    joined_data = data[0];
    map_data = data[3];
    ...
    const control_panel = new Controls(
      joined_data,
      "#date_slider",
      control_panel_dispatcher
    );
    const overview = new Overview(
      overviewFilter(joined_data, secondary_selector),
      "#overview",
      control_panel_dispatcher,
      secondary_selector
    );
    ...
    const detail = new Detail(
      detailFilter(joined_data, secondary_selector, null),
      "#detail",
      control_panel_dispatcher,
      secondary_selector,
      plane_lookup
    ...
    d3.selectAll("input.controlbox").on("click", function () {
      ...
      joined_data = controlBoxFilter(
        full_data,
        visualizations_view_2,
        checkboxes,
        secondary_selector,
        date,
        overview,
    ...
    const phaseGroupedData = d3.groups(
      joined_data,
...
window.onscroll = function (e) {
  let startPosFlightContainer = d3
    .select("svg#flight-path")
    .node()
  ...
  if (startPosFlightContainer < diff + diff2) {
    d3.select(".title-2")
      ...
      .style("top", marginFixed + "px");

    d3.select(".top-view")
      .style("position", "sticky")
      .style("top", diff + marginFixed + "px");
    d3.select("svg#flight-path")
      .style("position", "sticky")
      .style("top", diff + diff2 + marginFixed);
  }
};

function controlBoxFilter(
  data,
  views,
  checkboxes,
  secondary_select,
  ...
  if (checkboxes[2] === false) {
    // don't include amateur built
    new_Data = new_Data.filter((ele) => {
...
function controlFilter(data, attribute) {
  let dataGrouped = d3.groups(data, (d) =>
  ...
  switch (attribute) {
    case "Total Fatal Injuries":
      dataAttributed = dataGrouped.map((ele) => {
        return [
          ele[0],
          ele[1].map((val) => val[attribute]).reduce((acc, cur) => acc + cur),
        ];
      });
      break;
    case "Most Destroyed Craft":
      dataAttributed = dataGrouped.map((ele) => {
        return [
          ele[0],
          ele[1]
            .map((val) => val["Aircraft Damage"])
            .filter((cur) => cur.toLowerCase() === "destroyed").length,
        ];
      });
      break;
    ...
    case "Fatal to non-Fatal ratio":
      dataAttributed = dataGrouped.map((ele) => {
        return [
          ele[0],
          ele[1]
            .map((e) => e["Total Fatal Injuries"])
            .reduce((acc, cur) => acc + cur) /
            (ele[1]
              .map((e) => e["Total Serious Injuries"])
              .reduce((acc, cur) => acc + cur) +
              ele[1]
                .map((e) => e["Total Minor Injuries"])
                .reduce((acc, cur) => acc + cur)),
        ];
      });
      dataAttributed = dataAttributed.filter(
        (e) => e[1] !== Infinity && !isNaN(e[1])
      );
      break;
    case "Serious Injuries":
      dataAttributed = dataGrouped.map((ele) => {
        return [
          ele[0],
          ele[1]
            .map((val) => val["Total Serious Injuries"])
            .reduce((acc, cur) => acc + cur),
        ];
      });
      break;
    case "Minor Injuries":
      dataAttributed = dataGrouped.map((ele) => {
        return [
          ele[0],
          ele[1]
            .map((val) => val["Total Minor Injuries"])
            .reduce((acc, cur) => acc + cur),
        ];
      });
      break;
